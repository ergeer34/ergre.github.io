-- Pet Farm Script with WindUI
-- WindUI loads first, then game logic
...
if Cash <= 10000 and not HasTradeLic then
    ...
    local success, err = pcall(function()
        ...
        local args = { [1] = "theme_color", [2] = "red" }
        task.wait(2)
        game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("SettingsAPI/SetSetting"):FireServer(unpack(args))
        task.wait(2)
        ...
        task.wait(2)
        game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("HousingAPI/PushFurnitureChanges"):FireServer({})
        task.wait(2)
        game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("HousingAPI/SendHousingOnePointOneLog"):FireServer("edit_state_exited", {})
        task.wait(2)
    ...
    if not success then warn("Error in first task: " .. tostring(err)) end
    ...
    if not success then warn("Error in second task: " .. tostring(err)) end
...
local function getCurrentMoney()
    ...
    if currentMoney == nil then return 0 end
    return currentMoney
end
        ...
        for _, child in pairs(furnitureFolder:GetChildren()) do
            if child:IsA("Folder") then
                for _, grandchild in pairs(child:GetChildren()) do
                    if grandchild:IsA("Model") and grandchild.Name == furnitureName then
                        return grandchild:GetAttribute("furniture_unique")
                    end
                end
            end
        end
...
local function getHighestLevelPet()
    for i, v in pairs(fsys.get("inventory").pets) do
        if levelOfPet < v.properties.age and v.kind ~= "practice_dog" then
            ...
            petToEquip = v.unique
            if levelOfPet >= 6 then return petToEquip end
        end
    end
    return petToEquip
end
...
local function getAilments(tbl)
    ...
    for key, value in pairs(tbl) do
        if equipManagerPets and equipManagerPets[1] and key == equipManagerPets[1].unique then
...
local function equipPet()
    checkHouse()    
    local success, fsysLocal = pcall(function()
        return require(game:GetService("ReplicatedStorage").ClientModules.Core.ClientData)
    end)
    if not success or not fsysLocal then return end
    
    local equipManager = fsysLocal.get("equip_manager")
    local equipManagerPets = equipManager and equipManager.pets or {}
    local inventory = fsysLocal.get("inventory")
...
local function createPlatform()
    ...
    if existingPlatform then existingPlatform:Destroy() end
...
local function buyItems()
    local args = { [1] = "strollers", [2] = "stroller-default", [3] = { ["buy_count"] = 1 } }
    game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("ShopAPI/BuyItem"):InvokeServer(unpack(args))
    local args2 = { [1] = "toys", [2] = "squeaky_bone_default", [3] = { ["buy_count"] = 1 } }
...
local function buyItem(itemName)
    local args = { [1] = "food", [2] = itemName, [3] = { ["buy_count"] = 1 } }
    game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("ShopAPI/BuyItem"):InvokeServer(unpack(args))
end
...
local function getFoodID(itemName)
    ...
    for key, value in pairs(ailmentsData) do
        if value.id == itemName then return key end
    end
    return nil
end
...
local function hasTargetAilment(targetKind)
    ...
    for _, ailment in pairs(ailments) do
        if ailment.kind == targetKind then return true end
    end
    return false
end
...
local function EatDrink(isEquippedPet)
    if isEquippedPet then equipPet() end
    task.wait(1)
    if table.find(PetAilmentsArray, "hungry") then
        ...
        if getgenv().FoodID then
            ...
            local t = 0
            repeat task.wait(1) t = t + 1 until not hasTargetAilment("hungry") or t > 60
            local args = { [1] = getgenv().fsys.get("pet_char_wrappers")[1].pet_unique }
    ...
    if table.find(PetAilmentsArray, "thirsty") then
        ...
        if getgenv().WaterID then
            ...
            local t = 0
            repeat task.wait(1) t = t + 1 until not hasTargetAilment("thirsty") or t > 60
            local args = { [1] = getgenv().fsys.get("pet_char_wrappers")[1].pet_unique }
            game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("AdoptAPI/EjectBaby"):FireServer(unpack(args))
        else
            if startingMoney > 80 then
...
local function EatDrinkSafeCall(isEquippedPet)
    ...
    while not success do
        success, err = pcall(function() EatDrink(isEquippedPet) end)
        if not success then warn("Error occurred: ", err) task.wait(1) end
    end
end
...
local function claimAdventCalendar()
    pcall(function()
        ...
        local admday = day - 1
        game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("WinterEventAPI/AdventCalendarTryTakeReward"):InvokeServer(admday)
    end)
end
...
local function startPetFarm()
    ...
    task.wait(5)
    buyItems()
    game:GetService("ReplicatedStorage").API:FindFirstChild("LocationAPI/SetLocation"):FireServer("MainMap", game:GetService("Players").LocalPlayer, "Snow")
    ...
    task.spawn(function()
        while getgenv().PetFarmGuiStarter do
            for _, obj in ipairs(workspace:GetDescendants()) do
                if obj.Name == "BucksBillboard" or obj.Name == "XPBillboard" then obj:Destroy() end
            end
            task.wait(0.5)
        end
    end)
    ...
    while getgenv().PetFarmGuiStarter do
        ...
        if table.find(PetAilmentsArray, "sleepy") then
            ...
            if getgenv().PetBedID then
                ...
                local t = 0
                repeat task.wait(1) t = t + 1 until not hasTargetAilment("sleepy") or t > 60
                local args = { [1] = getgenv().fsys.get("pet_char_wrappers")[1].pet_unique }
                game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("AdoptAPI/EjectBaby"):FireServer(unpack(args))
            else
                startingMoney = getCurrentMoney()
        ...
        if table.find(PetAilmentsArray, "dirty") then
            ...
            if getgenv().ShowerID then
                ...
                local t = 0
                repeat task.wait(1) t = t + 1 until not hasTargetAilment("dirty") or t > 60
                local args = { [1] = getgenv().fsys.get("pet_char_wrappers")[1].pet_unique }
                game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("AdoptAPI/EjectBaby"):FireServer(unpack(args))
            else
                startingMoney = getCurrentMoney()
        ...
        if table.find(PetAilmentsArray, "toilet") then
            ...
            if getgenv().ToiletID then
                ...
                local t = 0
                repeat task.wait(1) t = t + 1 until not hasTargetAilment("toilet") or t > 60
                local args = { [1] = getgenv().fsys.get("pet_char_wrappers")[1].pet_unique }
                game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("AdoptAPI/EjectBaby"):FireServer(unpack(args))
        ...
        if table.find(PetAilmentsArray, "mystery") then
            ...
            for _, action in ipairs(actions) do
                local ClientData = require(game:GetService("ReplicatedStorage").ClientModules.Core.ClientData)
                local args = { ClientData.get_data()[game.Players.LocalPlayer.Name].equip_manager.pets[1].unique, "mystery", 1, action }
                game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("AilmentsAPI/ChooseMysteryAilment"):FireServer(unpack(args))
                task.wait(2)
            end
            local t = 0
            repeat task.wait(1) t = 1 + t until not hasTargetAilment("mystery") or t > 60
            local args = { [1] = getgenv().fsys.get("pet_char_wrappers")[1].pet_unique }
            game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("AdoptAPI/EjectBaby"):FireServer(unpack(args))
        ...
        if table.find(PetAilmentsArray, "play") then
            ...
            for i = 1, 3 do
                ...
                for i, v in pairs(fsys.get("inventory").toys) do
                    if v.id == "squeaky_bone_default" then ToyToThrow = v.unique end
                end
            ...
            local t = 0
            repeat task.wait(1) t = 1 + t until not hasTargetAilment("play") or t > 60
            local args = { [1] = getgenv().fsys.get("pet_char_wrappers")[1].pet_unique }
            game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("AdoptAPI/EjectBaby"):FireServer(unpack(args))
        ...
        if table.find(PetAilmentsArray, "walk") then
            ...
            Humanoid.WalkSpeed = walkDistance / walkDuration
            ...
            local t = 0
            repeat if not getgenv().PetFarmGuiStarter then break end t = 1 + t task.wait(1) until not hasTargetAilment("walk") or t > 60
        ...
        if table.find(PetAilmentsArray, "ride") then
            ...
            for i,v in pairs(fsys.get("inventory").strollers) do
                if v.id == 'stroller-default' then strollerUnique = v.unique end   
            end
            
            local args = { [1] = strollerUnique, [2] = { ["use_sound_delay"] = true, ["equip_as_last"] = false } }
            game:GetService("ReplicatedStorage"):WaitForChild("API"):WaitForChild("ToolAPI/Equip"):InvokeServer(unpack(args))

            pcall(function()
                local args = { game:GetService("Players"):WaitForChild(workspace:WaitForChild("PlayerCharacters"):GetChildren()[1].Name), workspace:WaitForChild("Pets"):GetChildren()[1], game:GetService("Players").LocalPlayer.Character:WaitForChild("StrollerTool"):WaitForChild("ModelHandle"):WaitForChild("TouchToSits"):WaitForChild("TouchToSit") }
            ...
            local t = 0
            repeat t = 1 + t if not getgenv().PetFarmGuiStarter then break end task.wait(1) until not hasTargetAilment("ride") or t > 60

            pcall(function()
                local argsUnequip = { [1] = strollerUnique, [2] = { ["use_sound_delay"] = true } }
    ...
    local function buildBuckets(activeSet)
        ...
        for k, p in pairs(inv() or {}) do
            ...
            if uid and kind and age == 6 and not activeSet[uid] and not locked then
                ...
                if arr then arr[#arr + 1] = uid else bucket[kind] = { uid } end
            end
        end
    ...
    local function fuseBuckets(buckets, label)
        ...
        for kind, arr in pairs(buckets) do
            ...
            for i = 1, n, 4 do
                ...
                if fuse4(a, b, c, d) then groups = groups + 1 else break end
                task.wait(1.0)
            end
        end
        return groups
    ...
    while true do
        if getgenv().AutoFusionEnabled then
            pcall(function()
                ...
                if commit then
                    if commit:IsA("RemoteEvent") then commit:FireServer()
                    elseif commit:IsA("RemoteFunction") then commit:InvokeServer() end
                end
            end)
...
local Window = WindUI:CreateWindow({
    Folder = "PetFarm Scripts",
    Title = "PET FARM - WINDUI",
    Icon = "heart",
...
Tabs.Home:Paragraph({
    ...
    Desc = "Complete Adopt Me farming script with Pet/Baby Auto Farm, Winter 2025 Events, Neon/Mega Fusion, Anti-AFK & Anti-Rejoin."
})
...
Tabs.PetFarm:Paragraph({
    ...
    Desc = "Automatically handles all pet ailments: hungry, thirsty, sleepy, dirty, sick, play, walk, ride, mystery."
})
...
Tabs.PetFarm:Toggle({
    ...
    Callback = function(value)
        ...
        if value then task.spawn(startPetFarm) end
    end
})
...
Tabs.PetFarm:Button({
    ...
    Desc = "Creates a platform in the sky",
    Callback = function()
        createPlatform()
        teleportPlayerNeeds(0, 350, 0)
    end
...
Tabs.BabyFarm:Paragraph({
    ...
    Desc = "Handles baby ailments automatically."
})
...
Tabs.Winter:Paragraph({
    ...
    Desc = "Features for Winter 2025 event."
})

Tabs.Winter:Button({
    Title = "Claim Advent Calendar",
    Desc = "Claim today's advent reward",
    Callback = claimAdventCalendar
})

Tabs.Fusion:Paragraph({
    Title = "Neon & Mega Fusion",
    Desc = "Auto fuse pets into Neon/Mega when you have 4 of the same kind at full growth."
})

Tabs.Fusion:Toggle({
    Title = "Auto Fusion",
    Desc = "Automatically fuse pets",
    Value = false,
    Callback = function(value)
...
print("Pet Farm WindUI Script Loaded!")
